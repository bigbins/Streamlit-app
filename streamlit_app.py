...
s3 = st.sidebar.slider(label="坐标空间的标准方差(像素单位)，一般尽可能小", min_value=0, max_value=255, value=250)
...
s6 = st.sidebar.slider(label="铅笔画亮度参数，值越大，图像越亮", min_value=0.00, max_value=0.10, value=0.05)


if img_file_buffer is not None:
    image = np.array(Image.open(img_file_buffer))
    st.sidebar.image(image, caption=f"原始图片", use_column_width=True)
    ...
    with c1:
        st.subheader('轮廓图')
        st.image(custom_sketch_image, caption=f"轮廓图", use_column_width=True)

        st.subheader('漫画图')
        st.image(custom_cartonized_image, caption=f"漫画图", use_column_width=True)

    with c2:
        st.subheader('灰度漫画图')
        st.image(custom_cartonized_image_gray, caption=f"灰度漫画图", use_column_width=True)

        st.subheader('定制化图像')
        st.image(stylizated_image, caption=f"定制化图像", use_column_width=True)

    with c3:
        st.subheader('铅笔风格灰度图')
        st.image(sketch_gray, caption=f"铅笔风格灰度图", use_column_width=True)

        st.subheader('铅笔风格轮廓图')
        st.image(sketch_color, caption=f"铅笔风格轮廓图", use_column_width=True)
